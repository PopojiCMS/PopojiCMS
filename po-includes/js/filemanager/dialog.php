	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
            ...
            $numperpage = $num_per_page_conf;
			if(empty($_GET['page'])){
				$beginpage = 0;
				$_GET['page'] = 1;
			}else{
				$beginpage = ($_GET['page']-1) * $numperpage;
			}
			$activepage = isset($_GET['page']) ? (int)$_GET['page'] : 1;

			if ($filter == '') {
			$filefiles = array_slice($files, $beginpage, $numperpage);
			} else {
			$filefiles = $files;
			}

			$files_prevent_duplicate = array();
		    foreach ($filefiles as $nu=>$file_array) {
			$file=$file_array['file'];
...
?>
	...
	<div class="row-fluid" style="bottom:0;">
		<div class="span12">
			<div class="pagination text-center" style="clear:both; padding:10px;">
				<ul>
					<?php
						$typepage = isset($_GET['type']) ? $_GET['type'] : '';
						$langpage = isset($_GET['lang']) ? $_GET['lang'] : '';
						$popuppage = isset($_GET['popup']) ? $_GET['popup'] : '';
						$crossdomainpage = isset($_GET['crossdomain']) ? $_GET['crossdomain'] : '';
						$field_idpage = isset($_GET['field_id']) ? $_GET['field_id'] : '';
						$relative_urlpage = isset($_GET['relative_url']) ? $_GET['relative_url'] : '';
						$akeypage = isset($_GET['akey']) ? $_GET['akey'] : '';
						$fldrpage = isset($_GET['fldr']) ? $_GET['fldr'] : '';
						$linkpage = "?type=".$typepage."&lang=".$langpage."&popup=".$popuppage."&crossdomain=".$crossdomainpage."&field_id=".$field_idpage."&relative_url=".$relative_urlpage."&akey=".$akeypage."&fldr=".$fldrpage;
						echo displayLinks($n_files, $numperpage, $activepage, $linkpage);
					?>
				</ul>
			</div>
		</div>
