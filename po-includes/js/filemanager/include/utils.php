...
function displayLinks($inumberresources, $numperpage, $activepage, $link){
	$numberPages = ceil($inumberresources / $numperpage);
	$output = "";

	$output .= "<li><a href='".$_SERVER['PHP_SELF'].$link."&page=1'>First</a></li>";

	if($activepage > 1){
		$prev = $activepage-1;
		$output .= "<li><a href='".$_SERVER['PHP_SELF'].$link.'&page='.$prev."'>Prev</a></li>";
	}else{
		$output .= "<li class='disabled'><a href='javascript:void(0)'>Prev</a></li>";
	}

	$angka = ($activepage > 10 ? "<li class='disabled'><a href='javascript:void(0)'>...</a></li>" : " ");
	for($i = $activepage-9; $i < $activepage; $i++){
		if ($i < 1)
		continue;
		$angka .= '<li><a href="'.$_SERVER['PHP_SELF'].$link.'&page='.$i.'">'.$i.'</a></li>';
	}
	$angka .= '<li class="active"><a href="javascript:void(0)">' . $i . '</a></li>';
	for($i = $activepage+1; $i < ($activepage+10); $i++){
		if ($i > $numberPages)
		break;
		$angka .= '<li><a href="'.$_SERVER['PHP_SELF'].$link.'&page='.$i.'">'.$i.'</a></li>';
	}
	$angka .= ($activepage+9 < $numberPages ? "<li class='disabled'><a href='javascript:void(0)'>...</a></li><li><a href='".$_SERVER['PHP_SELF'].$link.'&page='.$numberPages."'>".$numberPages."</a></li>" : " ");
	$output .= $angka;

	if($activepage < $numberPages){
		$next = $activepage+1;
		$output .= "<li><a href='".$_SERVER['PHP_SELF'].$link.'&page='.$next."'>Next</a></li>";
	}else{
		$output .= "<li class='disabled'><a href='javascript:void(0)'>Next</a></li>";
	}

	$output .= "<li><a href='".$_SERVER['PHP_SELF'].$link.'&page='.$numberPages."'>Last</a></li>";
